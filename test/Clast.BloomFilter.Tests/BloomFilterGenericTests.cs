...
public class BloomFilterGenericTests
{
    ...
    [Fact]
    public void Hash64_Guid_MatchesToByteArrayLayout()
    {
        // Pins the Guid hash to the documented little-endian ToByteArray
        // layout, so the zero-alloc Unsafe.WriteUnaligned path on
        // netstandard2.0 stays bit-identical to the TryWriteBytes path on
        // net8+. If a TFM ever diverges, this fails on that TFM.
        var g = new Guid("12345678-1234-1234-1234-123456789ABC");
        var expected = XxHash64.HashToUInt64(g.ToByteArray());
        Assert.Equal(expected, Hash64.Default<Guid>().Hash(g));
    }
    ...
    public void Builder_RoundTripsViaRawBytes()
    {
        ...
        var rehydrated = new BloomFilter<int>(new SplitBlockBloomFilter(bytes));
