...
public sealed class SplitBlockBloomFilterBuilder
{
    ...
    /// <summary>Zero-copy view of the current bitset bytes.</summary>
    public ReadOnlySpan<byte> Data => _data;

    /// <summary>Returns an independent copy of the current bitset bytes.</summary>
    public byte[] ToArray() => (byte[])_data.Clone();

    /// <summary>
    /// Materializes a probe-only <see cref="SplitBlockBloomFilter"/> over the current bitset.
    /// The returned filter shares this builder's buffer, so further <see cref="Add"/>
    /// or <see cref="AddHash"/> calls will be visible through it.
    /// </summary>
