...
#if NETSTANDARD2_0
        // Guid.TryWriteBytes(Span<byte>) isn't on netstandard2.0. The struct's
        // in-memory layout matches the TryWriteBytes output on little-endian
        // (which is every supported .NET architecture), so a raw write gives
        // identical bytes — and therefore identical hashes — across TFMs.
